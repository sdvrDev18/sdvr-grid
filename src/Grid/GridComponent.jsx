...
export default class GridComponent extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      filterError: false,
      selectedRows:
        this.props.checkBoxArray && this.props.checkBoxArray.length > 0
          ? this.props.checkBoxArray.map(
              el => this.props.rowData.filter(val => val.id === el)[0]
            )
          : [],
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (diffArray && diffArray.length > 0) {
      this.setState({
        ...
        checkBoxArray:
          nextProps.checkBoxArray && nextProps.checkBoxArray.length > 0
            ? nextProps.checkBoxArray
            : [],
        totalRows: nextProps.totalRows,
        selectAllCheckbox:
          nextProps.checkBoxArray && nextProps.checkBoxArray.length === 0
            ? false
            : this.state.selectAllCheckbox
      });
      if (nextProps.rowData.length !== this.state.rowsPerPage)
        this.setPageData(nextProps.rowData, nextProps.totalRows);
    }
  }

  componentDidMount() {
    if (this.props.rowData.length != this.state.rowsPerPage)
      this.setPageData(this.props.rowData);
    if (
      this.props.checkBoxArray &&
      this.props.checkBoxArray.length === this.props.rowData.length
    )
  ...
  setPageData = (data, totalRows) => {
    ...
    if (this.props.paginationType === "server-side") {
      this.setState({
        ...
        pageNumber:
          totalRows > this.state.rowsPerPage ? this.state.pageNumber : 1
      });
    ...
    if (operator === "Contains") {
        ...
        this.props.rowData && value !== ""
          ? this.props.rowData.filter(val => {
              if (val[column] && Number(val[column])) {
                ...
                return (
                  val[column] &&
                  val[column].toLowerCase().includes(value.toLowerCase())
                );
              }
            })
          : this.props.rowData;
      ...
      filteredTable =
        this.props.rowData && value !== ""
          ? this.props.rowData.filter(val => {
              if (val[column] && Number(val[column]))
                return val[column] && val[column].toString() === value;
              else
                return (
                  val[column] &&
                  val[column].toLowerCase() === value.toLowerCase()
                );
            })
          : this.state.rowData;
    }
    ...
    if (this.props.paginationType === "server-side") {
      ...
      const paginatedRowData =
        this.state.currentPageData.length !== 0
          ? this.state.currentPageData.slice(
              (event - 1) * this.state.rowsPerPage,
              event * this.state.rowsPerPage
            )
          : this.props.rowData.slice(
              (event - 1) * this.state.rowsPerPage,
              event * this.state.rowsPerPage
            );
      this.setState({
  ...
  handlePageNumberInput = e => {
    ...
    if (Math.sign(pageInput) === 1) {
      this.setState({
        ...
        pageSearchInput: pageInput,
        isClearPageInput: true
      });
  ...
  clearPageSearch = () => {
    if (this.props.paginationType === "server-side") {
      this.setState({ pageNumber: 1 }, () =>
        this.props.handleServersidePagination(this.state.pageNumber)
      );
    }
    this.setPageData(this.props.rowData);
  ...
  handleRowClick = (rowIndex, selectedElement) => {
    if (selectedElement.props && selectedElement.props.name === "tableButton") {
      ...
      this.props.collapsibleData(
        rowIndex,
        this.state.rowData[rowIndex].id,
  ...
  sortUp = index => {
    ...
    let sortedObj = this.state.filterApplied
      ? [...this.state.currentPageData]
      : [...this.props.rowData];
    let newSortedObj = sortedObj.filter(
      val => val[sortColumn] && val[sortColumn] !== ""
    );
    let nullSortedObj = sortedObj.filter(
      val => !val[sortColumn] || val[sortColumn] === ""
    );
    ...
    newSortedObj.sort(function(first, second) {
      const firstValue = !Number(first[sortColumn])
        ? first[sortColumn]
          ? first[sortColumn].toLowerCase()
          : ""
        : Number(first[sortColumn]); // ignore upper and lowercase
      const secondValue = !Number(second[sortColumn])
        ? second[sortColumn]
          ? second[sortColumn].toLowerCase()
          : ""
        : Number(second[sortColumn]); // ignore upper and lowercase
  ...
  handleCheckbox = (event, rowIndex) => {
    ...
    if (event.target.checked === true) {
      ...
      this.props.paginationType === "client-side" &&
        this.props.rowData.length === checkArray.length &&
        this.setState({ selectAllCheckbox: true });
      this.setState({
        checkBoxArray: checkArray
      });
      ...
      checkArray.forEach(
        (val, index) => val === checkedRow.id && checkArray.splice(index, 1)
      );
      this.props.rowData.length !== checkArray.length &&
    ...
    return (
      <React.Fragment>
        <table
          ...
          className="table list-view mt15"
        >
          <thead>
            <tr>
              {this.props.checkbox === true && (
                <th
                  key={`multiCheckbox-${uuidv4()}`}
                  style={{
                    width: "10px",
                    paddingTop: 0,
                        ...
                        >
                          ...
                          {cols.sortable === true && (
                            <span style={{ display: "inline-grid" }}>
                              ...
                              />
                              <span
                                className="table-sort-down-button"
            ...
            {this.state.rowData &&
              this.state.rowData.map((rows, rowIndex) => (
                <React.Fragment key={uuidv4()}>
                  <tr key={uuidv4()} className="grid-row-style">
                    {this.props.checkbox === true && (
                      <td
                        key={uuidv4()}
                        style={{ borderRight: "0.5px solid lightgrey" }}
                      >
                        <input
                          key={uuidv4()}
                          onClick={event =>
                            this.handleCheckbox(event, rowIndex)
                          }
                          checked={this.state.checkBoxArray.includes(rows.id)}
                          className="table-checkbox"
                          type="checkbox"
                        />
                      </td>
                    ...
                    {columnsArray.map(el => {
                      return (
                        <td
                          key={uuidv4()}
                          onClick={() =>
                            this.handleRowClick(rowIndex, rows[el])
                          }
                          style={{
                            textAlign: "center",
                            borderRight: "0.5px solid lightgrey"
                  ...
                  {this.state.collapsibleId === rows.id && (
                    <tr
                      key={uuidv4()}
                      className={
                        this.props.collapsible && this.state.isCollapseOpen
                          ? "collapsible_visible"
                          : "collapsible_invisible"
                      }
                    >
        ...
        <div className="table-pagination">
          <Pagination
            hideDisabled
            totalItemsCount={this.state.totalRows ? this.state.totalRows : 0}
            activePage={this.state.pageNumber}
            onChange={this.handlePageChange}
            itemsCountPerPage={
              this.state.rowsPerPage ? this.state.rowsPerPage : 8
            }
            pageRangeDisplayed={
              this.props.pageRangeDisplayed ? this.props.pageRangeDisplayed : 5
            }
          ...
          <div style={{ width: "15%", alignSelf: "center" }}>
            <input
              ...
              placeholder="Pagenum..."
              value={
                this.state.pageSearchInput === 0
                  ? ""
                  : this.state.pageSearchInput
              }
            />
